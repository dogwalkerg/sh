...
wget -N --no-check-certificate "https://raw.githubusercontent.com/dogwalkerg/sh/master/lxiaoya.zip"
unzip lxiaoya.zip
cd lxiaoya
mv * .[^.]* /www/wwwroot/$website/
cd ..
rm -rf lxiaoya.zip lxiaoya/
...
echo -e "${Info} 正在导入数据库"
cd sql/
mysql -u$mysqlusername -p$mysqlpassword $mysqldatabase < glzjin_guf.sql >/dev/null 2>&1
...
echo -e "${Info} 默认生成的管理员用户名：admin 密码为1234567890"
